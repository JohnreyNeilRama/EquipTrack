...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Department metadata list initialized to empty
            const departments = [];
            ...
            function renderDepartmentGrid() {
                ...
                if (!departments || departments.length === 0) {
                    deptGrid.innerHTML = `
                        <div style="grid-column: 1 / -1; text-align: center; color: var(--text-muted); padding: 48px 24px; background: var(--bg-card); border-radius: 12px; border: 1px dashed var(--border-color);">
                            <i class="fa-solid fa-building-circle-exclamation" style="font-size: 32px; color: var(--text-muted); margin-bottom: 12px; display: block;"></i>
                            <h4 style="font-size: 16px; font-weight: 600; color: var(--text-main); margin-bottom: 4px;">No Departments Available</h4>
                            <p style="font-size: 14px; color: var(--text-muted); margin: 0;">No registered department accounts are currently available for equipment monitoring.</p>
                        </div>
                    `;
                    return;
                }
