...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_department') {
    ...
    $dept_head  = trim($_POST['department_head'] ?? '');

    if (empty($dept_name) || empty($dept_code) || empty($college) || empty($dept_head)) {
        echo json_encode(['success' => false, 'message' => 'All fields (Department Name, Department Code, College, Department Head) are required.']);
        exit;
    }

    // Check if department name or code already exists in department table
    $stmtChk = $conn->prepare("SELECT department_id FROM department WHERE department_name = ? OR department_code = ?");
    if ($stmtChk) {
        $stmtChk->bind_param("ss", $dept_name, $dept_code);
        $stmtChk->execute();
        $resChk = $stmtChk->get_result();
        if ($resChk && $resChk->num_rows > 0) {
            echo json_encode(['success' => false, 'message' => 'A department with this name or code already exists.']);
            exit;
        }
    }

    $stmtIns = $conn->prepare("INSERT INTO department (department_name, department_code, college, department_head) VALUES (?, ?, ?, ?)");
    if ($stmtIns) {
        $stmtIns->bind_param("ssss", $dept_name, $dept_code, $college, $dept_head);
        if ($stmtIns->execute()) {
    ...
    echo json_encode(['success' => false, 'message' => 'Failed to save department: ' . $conn->error]);
    exit;
}
...
<head>
    ...
    <link rel="stylesheet" href="css/adminmonitoring.css?v=<?php echo time(); ?>">
...
<body>
                    ...
                    <?php if (!empty($dbDepartments)): ?>
                        <?php foreach ($dbDepartments as $dept): ?>
                            <?php 
                                ...
                                $imgUrl = $dept['image'] ?? '../images/logo_only.png';
                                $sub = $deptCode ? ($deptCode . ' • ' . $collegeName) : $collegeName;
                            ?>
                            <div class="dept-card" onclick="selectDepartment(<?php echo htmlspecialchars(json_encode($deptTitle)); ?>)">
                                <div class="dept-logo-wrapper">
                                    <img src="<?php echo htmlspecialchars($imgUrl); ?>" alt="<?php echo htmlspecialchars($deptTitle); ?>" class="dept-logo-img">
                                </div>
                                <h4 class="dept-card-title"><?php echo htmlspecialchars($deptTitle); ?></h4>
                                <p class="dept-card-subtitle"><?php echo htmlspecialchars($sub); ?></p>
                                <?php if (!empty($headName)): ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            let dbDepts = <?php echo json_encode($dbDepartments); ?>;
